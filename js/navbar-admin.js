import { onAuthStateChanged, getAuth, signOut } from 'https://www.gstatic.com/firebasejs/10.5.2/firebase-auth.js';
...
function restrictNavbarForEmployees(user) {
	...
	getDoc(docRef).then(userSnap => {
		...
		if (getAuth().currentUser.email == "healthhub.siaton@gmail.com") {
			document.querySelector("#navDashboard").classList.toggle("d-none", true);
		}
	});
}